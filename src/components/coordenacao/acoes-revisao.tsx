...
export function AcoesRevisao({ artigoId }: { artigoId: string }) {
  ...
  return (
    <>
      ...
      {/* sheet de ajustes */}
      {sheet && (
        <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: "rgba(17,17,17,.45)" }} onClick={() => setSheet(false)}>
          <div
            className="w-full max-w-lg rounded-t-3xl p-5"
            style={{ background: C.fundo }}
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-[19px]" style={{ fontFamily: F.serif, fontWeight: 600 }}>
              Pedir ajustes
            </h2>
